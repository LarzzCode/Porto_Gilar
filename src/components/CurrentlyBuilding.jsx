...
const builds = [
  {
    ...
    status: "Active now",
    description:
      "Evolving this portfolio into a stronger recruiter and client-facing product through case studies, responsive UX, analytics, and clearer conversion paths.",
...
const CurrentlyBuilding = () => {
    ...
    const loadActivity = async () => {
      try {
        const response = await fetch(
          "https://api.github.com/users/LarzzCode/repos?sort=pushed&per_page=100",
          {
  ...
  const activeCount = useMemo(
    () => builds.filter((build) => build.status === "Active now").length,
    [],
  );
  ...
  return (
    <div className="mt-20 md:mt-28 border-t border-white/5 pt-14 md:pt-20">
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-8 md:mb-10">
        ...
        >
          <div className="inline-flex items-center gap-2 text-emerald-300/90 font-mono text-xs md:text-sm tracking-wider mb-3">
            <Radar size={15} /> NOW / BUILD LOG
          </div>
          <h3 className="text-3xl md:text-5xl font-bold text-white tracking-tight">
            What I&apos;m <span className="text-indigo-400">building now.</span>
          </h3>
          <p className="text-slate-500 mt-3 max-w-2xl text-sm md:text-base leading-relaxed">
            A small pulse of the products I am actively improving or have recently shipped. GitHub activity is refreshed automatically when available.
          </p>
        </motion.div>
        ...
        <div className="flex items-center gap-3 text-xs font-mono text-slate-500">
          <span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/15 bg-emerald-400/5 px-3 py-2 text-emerald-300/80">
            ...
            {activeCount} active build
          </span>
          <span className="hidden sm:inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-2">
            <Activity size={14} /> GitHub {activityState === "ready" ? "connected" : activityState === "loading" ? "syncing" : "fallback"}
          </span>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4 md:gap-5">
        {builds.map((build, index) => {
          ...
          const updated = formatRelativeDate(activity?.pushedAt);
          ...
          return (
            <motion.article
              ...
              className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/8 bg-[#0d0d0f] p-5 md:p-6 transition-all duration-300 hover:-translate-y-1 hover:border-indigo-400/25 hover:bg-[#101014]"
            >
              ...
              <p className="relative text-sm leading-relaxed text-slate-400 mb-6">
                {build.description}
              </p>
              ...
              <div className="relative mt-auto border-t border-white/5 pt-4">
                <div className="flex min-h-5 items-center gap-2 text-xs text-slate-500 mb-4">
                  ...
                  {updated ? (
                    <span>GitHub updated {updated}</span>
                  ) : activityState === "loading" ? (
                    <span>Checking latest GitHub activity…</span>
                  ) : (
                    <span>Public repository available</span>
                  )}
                  {activity?.language && (
                    <span className="ml-auto text-slate-600">{activity.language}</span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <a
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-xs font-semibold text-slate-300 transition-colors hover:bg-white/10 hover:text-white"
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-3 py-2.5 text-xs font-semibold text-white transition-colors hover:bg-indigo-500"
                  >
