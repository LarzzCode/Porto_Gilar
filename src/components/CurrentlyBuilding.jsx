...
const CurrentlyBuilding = () => {
    ...
    () => [
      {
        ...
        status: isId ? "Aktif sekarang" : "Active now",
        active: true,
        description: isId
      ...
      },
      {
        ...
        status: isId ? "Baru dirilis" : "Recently shipped",
        active: false,
        description: isId
  ...
  const copy = isId
    ? {
        ...
        titleB: "bangun.",
        intro: "Cuplikan kecil dari produk yang sedang saya tingkatkan atau baru saja saya rilis. Aktivitas GitHub diperbarui otomatis ketika tersedia.",
        active: "build aktif",
        github: "GitHub",
        connected: "terhubung",
    ...
    : {
        ...
        titleA: "What I'm ",
        titleB: "building now.",
        intro: "A small pulse of the products I am actively improving or have recently shipped. GitHub activity is refreshed automatically when available.",
        active: "active build",
        github: "GitHub",
        connected: "connected",
        syncing: "syncing",
  ...
  const activeCount = useMemo(() => builds.filter((build) => build.active).length, [builds]);
          ...
          return (
            <motion.article key={build.repo} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.45, delay: index * 0.08 }} className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/8 bg-[#0d0d0f] p-5 md:p-6 transition-all duration-300 hover:-translate-y-1 hover:border-indigo-400/25 hover:bg-[#101014]">
              ...
              <div className="relative flex items-start justify-between gap-4 mb-5">
                <div>
                  <span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] ${build.active ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-300" : "border-indigo-400/20 bg-indigo-400/10 text-indigo-300"}`}>{build.status}</span>
